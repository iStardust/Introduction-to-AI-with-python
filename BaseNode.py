...
class sigmoid(Node):
    ...
    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        sigx=self.cache[-1]
        return -np.multiply(grad, (1-sigx)*sigx)
        raise NotImplementedError        
...
class Linear(Node):
    ...
    def backcal(self, grad):
        ...
        weight=self.params[0]
        bias=self.params[1]
        wgrad=np.transpose(X)@grad
        bgrad=np.sum(grad,axis=0)
        self.grad.append(wgrad)
        self.grad.append(bgrad)
        return grad@np.transpose(weight)
        raise NotImplementedError
...
class LogSoftmax(Node):
    ...
    def backcal(self, grad):
        # TODO: YOUR CODE HERE
        return grad - np.multiply(grad.sum(axis=self.dim, keepdims=True), self.cache[-1])
        raise NotImplementedError
...
class CrossEntropyLoss(Node):
    ...
    def cal(self, X):
        ...
        y=self.y
        self.cache.append(X)
        succ_p=np.take_along_axis(X, np.expand_dims(y, axis=-1), axis=-1)
        return -np.average(np.log(succ_p))
    ...
    def backcal(self, grad):
        ...
        succ_p=np.take_along_axis(X, np.expand_dims(y, axis=-1), axis=-1)
        num=y.shape[0]
        negavg_p=1/(-num*succ_p)
        np.put_along_axis(ret, np.expand_dims(y, axis=-1), 1, axis=-1)
        return grad * ret * negavg_p
        raise NotImplementedError