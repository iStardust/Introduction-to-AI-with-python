...
def cornersHeuristic(state, problem):
    ...
    This function should always return a
     number that is a lower bound on the
    ...
    "*** YOUR CODE HERE ***"
    #到最近的一个角落c的曼哈顿距离加上c到剩余角落的曼哈顿距离之和（递归）
    if len(Visited_Corners)!=4:
        nearest_mdis=999999
        nearest_corner=corners[0]
        for corner in corners:
            if corner not in Visited_Corners:
                tmp_mdis=abs(node[0]-corner[0])+abs(node[1]-corner[1])
                if tmp_mdis<nearest_mdis:
                    nearest_corner=corner
                    nearest_mdis=tmp_mdis
        h_sum+=nearest_mdis
        newstate=(nearest_corner,Visited_Corners+[nearest_corner])
        h_sum+=cornersHeuristic(newstate,problem)
